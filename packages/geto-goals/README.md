...
```text
/goal set Migrate the API client to v3; update all call sites; run unit and integration
      tests; keep going until they pass. --done "all tests green" --max-iterations 30
```

## Tool

`goal_report(status: "progress"|"done"|"blocked", note)` — the agent's progress channel. Writing it updates the durable goal file; the loop continues while the goal is `active`.

## The loop

- `before_agent_start` injects the goal (text, definition of done, last/next step, recent progress, iteration/budget counts) into every turn's system prompt.
- `agent_settled` re-triggers a follow-up turn while the goal is active **and** guards allow:
